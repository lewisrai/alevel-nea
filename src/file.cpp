...
/*
  ...
  std::ofstream - supports - std::ofstream::out - write
                           ...
                           - std::ofstream::trunc - deletes all contents of file
  before write
...
void LoadProgress(std::string saveFile, int &level, float &score, float &maxPlayersHealth,
                  std::array<int, 10> &upgrades) {
    std::string line = "";
...
void LoadSummary(std::string saveFile, std::string &str_level, std::string &str_score) {
    ...
    if (std::getline(file, line))
        str_level = "Levels Completed: " + line;
    if (std::getline(file, line))
        str_score = "Score: " + line;
    file.close();
}
...
void SaveProgress(std::string saveFile, int level, int score, int maxPlayersHealth,
                  const std::array<int, 10> &upgrades) {
    std::ofstream file(saveFile, std::ofstream::trunc);

    file << level << "\n";
    file << score << "\n";
    file << maxPlayersHealth << "\n";
    for (const int &upgrade : upgrades)
        file << upgrade << "\n";
    file.close();
}
...
void SortHighscores() {
    ...
    for (ScoreEntry &entry : highscores)
        if (entry.score != 0)
            file << entry.name << SEPARATOR << entry.score << "\n";
    file.close();
}
