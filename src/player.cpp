...
Player::Player(const SoundArray &sounds, bool player0, int keyUp, int keyLeft, int keyRight,
               int keyAttack0, int keyAttack1)
    : m_player0(player0) {
...
void Player::UpdateInput() {
    for (int i = 0; i < 5; i++)
        m_inputState[i] = IsKeyDown(m_keybinds[i]);
}

void Player::UpdateReceived(const BlockList &blocks, const MonsterList &monsters, const Wall &wall,
                            float &playersHealth) {
    // Magma Blocks
    for (const Block &block : blocks) {
        if (block.GetPositionX() < m_rect.x - MAX_BLOCK_LENGTH)
            continue;
        else if (block.GetTextureID() != ID_BLOCK_MAGMA)
            continue;
        else if (block.GetPositionX() > m_rect.x + MAX_BLOCK_LENGTH)
            break;

        // Check collision 1 "pixel" below centre bottom of player
        if (CheckCollisionPointRec({m_rect.x + m_rect.width, m_rect.y + m_rect.height + 1},
                                   block.GetRectangle())) {
    ...
    for (const Monster &monster : monsters) {
        ...
        if (monster.GetPositionX() < m_rect.x - MAX_BLOCK_LENGTH)
            continue;
        else if (monster.GetPositionX() > m_rect.x + MAX_BLOCK_LENGTH)
            continue;
...
Vector2 Player::GetCentre() const {
...
void Tether::Update(const PlayerArray &players, float &playersHealth) {
    ...
    const float distanceApart =
        (m_startPosition.x - m_endPosition.x) * (m_startPosition.x - m_endPosition.x) +
        (m_startPosition.y - m_endPosition.y) * (m_startPosition.y - m_endPosition.y);
