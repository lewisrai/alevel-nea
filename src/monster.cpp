...
void Monster::Draw(const TextureArray &textures, float cameraLimitLeft,
...
void Monster::UpdateAI(const BlockList &blocks, const PlayerArray &players) {
    ...
    const float p0Distance = (centre.x - p0Position.x) * (centre.x - p0Position.x) +
                             (centre.y - p0Position.y) * (centre.y - p0Position.y);
    const float p1Distance = (centre.x - p1Position.x) * (centre.x - p1Position.x) +
                             (centre.y - p1Position.y) * (centre.y - p1Position.y);

    const Vector2 closestPlayer = (p0Distance < p1Distance) ? p0Position : p1Position;

    // Update inputs
    if (centre.y > closestPlayer.y)
        m_inputState[JUMP] = true;

    if (centre.x > closestPlayer.x) {
        m_inputState[GO_LEFT] = true;
        if (IsBlockInFront(blocks, {centre.x - BLOCK_SIZE, centre.y + BLOCK_SIZE / 2}))
            m_inputState[JUMP] = true;
    } else {
...
void Monster::UpdateReceived(const PlayerArray &players, ProjectileList &projectiles, Wall &wall) {
    ...
    for (const Player &player : players) {
        ...
        if (player.GetPositionX() < m_rect.x - MAX_BLOCK_LENGTH)
            continue;
        else if (player.GetPositionX() > m_rect.x + MAX_BLOCK_LENGTH)
            continue;
    ...
    for (Projectile &projectile : projectiles) {
        // Skip if not near monster
        if (projectile.GetPositionX() < m_rect.x - MAX_BLOCK_LENGTH)
            continue;
        else if (projectile.GetPositionX() > m_rect.x + MAX_BLOCK_LENGTH)
            continue;
...
float Monster::GetDamageIfCollision(Rectangle rect) const {
    if (CheckCollisionRecs(m_rect, rect))
        return m_damage;
    else
        return 0;
}
...
bool IsBlockInFront(const BlockList &blocks, Vector2 checkPosition) {
    for (const Block &block : blocks) {
        ...
        if (block.GetPositionX() < checkPosition.x - MAX_BLOCK_LENGTH)
            continue;
        else if (block.GetPositionX() > checkPosition.x + MAX_BLOCK_LENGTH)
            break;

        if (CheckCollisionPointRec(checkPosition, block.GetRectangle()))
            return true;
    }
    return false;
}

void GenerateEnemies(const SoundArray &sounds, const BlockList &blocks, MonsterList &monsters,
                     int level) {
    ...
    for (const Block &block : blocks) {
        ...
        if (blockRect.width / BLOCK_SIZE < 3)
            continue;
        else if (blockRect.x < -BORDER_HORIZONTAL + LEVEL_PADDING * 2)
            continue;

        if (GetRandomValue(0, 99) < monsterThreshold)
            monsters.push_back(Monster(sounds, level, multiplier, blockRect.x + BLOCK_SIZE,
                                       blockRect.y - BLOCK_SIZE * 2));
    }
}
...
MonsterList RemoveDeadEnemies(MonsterList oldMonsters, float &score) {
    ...
    for (int i = 0; i < (int)oldMonsters.size(); i++) {
        if (oldMonsters[i].GetHealth() > 0)
            newMonsters.push_back(oldMonsters[i]);
        else
            score += oldMonsters[i].GetPoints();
    }
    return newMonsters;
}
