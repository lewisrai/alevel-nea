...
Projectile::Projectile(float radius, Vector2 position, float damage, float knockback, float speed)
    : m_radius(radius), m_damage(damage), m_knockback(knockback), m_speed(speed) {
    m_position = position;
}
...
void Projectile::Draw(float cameraLimitLeft, float cameraLimitRight) const {
    ...
    if (m_position.x < cameraLimitLeft)
        return;
    else if (m_position.x > cameraLimitRight)
        return;
...
void Projectile::UpdateCollision(const BlockList &blocks) {
    ...
    if (m_position.x < -BORDER_HORIZONTAL)
        m_active = false;
    else if (m_position.x > BORDER_HORIZONTAL)
        m_active = false;
    ...
    for (const Block &block : blocks) {
        ...
        if (block.GetPositionX() < m_position.x - MAX_BLOCK_LENGTH)
            continue;
        else if (block.GetPositionX() > m_position.x + MAX_BLOCK_LENGTH)
            continue;
...
ProjectileList RemoveFalseProjectiles(ProjectileList oldProjectiles) {
    ...
    for (int i = 0; i < (int)oldProjectiles.size(); i++)
        if (oldProjectiles[i].GetActive())
            newProjectiles.push_back(oldProjectiles[i]);
...
float Weapon::CheckSwordCollision(Rectangle rect) const {
    ...
    const Vector2 swordEndPosition = {(float)(m_rect.x + m_origin.y * sin(rotationRadians)),
                                      (float)(m_rect.y - m_origin.y * cos(rotationRadians))};

    Vector2 *temp = new Vector2;

    // Check line collision with the sides of the rectangle of monster
    if (CheckCollisionLines(swordStartPosition, swordEndPosition, {rect.x, rect.y},
                            {rect.x + rect.width, rect.y}, temp)) {
        // Top side
        delete temp;
        return m_damage;
    } else if (CheckCollisionLines(swordStartPosition, swordEndPosition, {rect.x, rect.y},
                                   {rect.x, rect.y + rect.height}, temp)) {
        // Left side
        delete temp;
        return m_damage;
    } else if (CheckCollisionLines(swordStartPosition, swordEndPosition,
                                   {rect.x + rect.width, rect.y},
                                   {rect.x + rect.width, rect.y + rect.height}, temp)) {
        // Right side
        delete temp;
        return m_damage;
    } else if (CheckCollisionLines(swordStartPosition, swordEndPosition,
...
void Weapon::Draw(const TextureArray &textures) const {
    if (m_active)
        DrawTexturePro(textures[m_textureID], m_textureSize, m_rect, m_origin, m_rotation, WHITE);
}

void Weapon::UpdateWeapon(bool facingLeft, float deltaTime, int keyAttack, Rectangle rect,
...
void Weapon::UpdateGun(bool facingLeft, float deltaTime, bool keyAttackDown, Rectangle rect,
                       ProjectileList &projectiles) {
    // Set Position
    ...
    // Update Cooldown
    if (!m_idle)
        m_cooldown -= m_reloadSpeed * deltaTime;
    else if (keyAttackDown) {
        PlaySound(m_soundAttack);
        projectiles.push_back(Projectile(m_bulletSize, {m_rect.x, m_rect.y}, m_damage, m_knockback,
                                         (facingLeft) ? -m_bulletSpeed : m_bulletSpeed));
        m_active = true;
        m_idle = false;
        m_cooldown = 100;
    }
...
void Weapon::UpdateSword(bool facingLeft, float deltaTime, bool keyAttackDown, Rectangle rect) {
    ...
    if (!m_idle)
        m_rotation += m_swingSpeed * deltaTime;
    else if (keyAttackDown) {
        PlaySound(m_soundAttack);
        m_active = true;
        m_idle = false;
