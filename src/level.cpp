...
Block::Block(int level, int textureID, Rectangle rect) : m_rect(rect), m_textureID(textureID) {
    if (m_textureID != ID_BLOCK_MAGMA)
        return;

    if (level > DIFFICULTY_INCREASE_2)
        m_damage = 15.0 / BASE_FPS;
    else
        m_damage = 10.0 / BASE_FPS;
}

// Public Methods //

void Block::Draw(const TextureArray &textures, float cameraLimitLeft,
                 float cameraLimitRight) const {
    // Skip if not near players
    if (m_rect.x < cameraLimitLeft)
        return;
    else if (m_rect.x > cameraLimitRight)
        return;

    for (int x = 0; x < m_rect.width; x += BLOCK_SIZE)
        DrawTexture(textures[m_textureID], m_rect.x + x, m_rect.y, WHITE);
}
...
// After adding terrain feature, returns new last block position of level for
// other obstacles
...
Vector2 AddTerrainFlat(BlockList &blocks, Vector2 currentPosition, int maxLength = 6) {
    ...
    blocks.push_back(Block(
        0, ID_BLOCK_DIRT, {currentPosition.x, currentPosition.y - BLOCK_SIZE, length, BLOCK_SIZE}));
...
Vector2 AddTerrainHillDown(BlockList &blocks, Vector2 currentPosition) {
    if (currentPosition.y > BORDER_VERTICAL - LEVEL_PADDING)
        return currentPosition;
    ...
    for (int i = 0; i < GetRandomValue(2, 4); i++) {
        blocks.push_back(
            Block(0, ID_BLOCK_DIRT,
                  {currentPosition.x - BLOCK_SIZE, currentPosition.y, length, BLOCK_SIZE}));
...
Vector2 AddTerrainHillUp(BlockList &blocks, Vector2 currentPosition) {
    ...
    currentPosition = AddTerrainFlat(blocks, currentPosition, 3);
    for (int i = 0; i < GetRandomValue(2, 4); i++) {
        blocks.push_back(Block(0, ID_BLOCK_DIRT,
                               {currentPosition.x - BLOCK_SIZE, currentPosition.y - BLOCK_SIZE * 2,
                                length, BLOCK_SIZE}));
        currentPosition.x += length - BLOCK_SIZE;
        currentPosition.y -= BLOCK_SIZE;
    }
...
Vector2 AddTerrainHoles(BlockList &blocks, Vector2 currentPosition) {
    ...
    for (int i = 0; i < GetRandomValue(2, 3); i++) {
        blocks.push_back(
            Block(0, ID_BLOCK_DIRT,
                  {currentPosition.x, currentPosition.y - BLOCK_SIZE, lengthPlatform, BLOCK_SIZE}));
        currentPosition.x += lengthPlatform;
        blocks.push_back(
            Block(0, ID_BLOCK_AIR,
                  {currentPosition.x, currentPosition.y - BLOCK_SIZE, lengthGap, BLOCK_SIZE}));
...
Vector2 AddTerrainMagma(BlockList &blocks, Vector2 currentPosition, int level) {
    ...
    for (int i = 0; i < GetRandomValue(4, 6); i++) {
        ...
        blocks.push_back(
            Block(level, (GetRandomValue(0, 1)) ? ID_BLOCK_MAGMA : ID_BLOCK_DIRT,
                  {currentPosition.x, currentPosition.y - BLOCK_SIZE, length, BLOCK_SIZE}));
        currentPosition.x += length;
    }
...
Rectangle GenerateLevelData(BlockList &blocks, int level) {
    ...
    while (currentPosition.x < BORDER_HORIZONTAL - LEVEL_PADDING) {
        ...
        if (randomValue < 12)
            currentPosition = AddTerrainHillDown(blocks, currentPosition);
        else if (randomValue < 24)
            currentPosition = AddTerrainHillUp(blocks, currentPosition);
        else if (randomValue < 30 && level > DIFFICULTY_INCREASE_1)
            currentPosition = AddTerrainMagma(blocks, currentPosition, level);
        else if (randomValue < 36 && level > DIFFICULTY_INCREASE_2)
            currentPosition = AddTerrainHoles(blocks, currentPosition);
        else
            currentPosition = AddTerrainFlat(blocks, currentPosition);
    }
...
void Wall::Draw(const TextureArray &textures) const {
    for (int y = m_rect.y; y < m_rect.y + m_rect.height; y += BLOCK_SIZE)
        DrawTexture(textures[m_textureID], m_rect.x, y, WHITE);

    DrawRectangleRec({m_rect.x - WALL_BEHIND, m_rect.y, WALL_BEHIND, m_rect.height},
                     {0, 0, 0, 128});
}
...
void Wall::Update(Camera2D camera, float deltaTime) {
    ...
    // Update y
    if (m_rect.y + WALL_HEIGHT / 2 - BLOCK_SIZE > camera.target.y)
        m_rect.y -= BLOCK_SIZE;
    else if (m_rect.y + WALL_HEIGHT / 2 + BLOCK_SIZE < camera.target.y)
        m_rect.y += BLOCK_SIZE;
}
