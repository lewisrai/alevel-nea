...
CameraView::CameraView(const Window &window)
    : m_baseZoom(window.width / BASE_WINDOW_WIDTH), m_maxX(window.width / 2),
      m_maxY(window.height / 2) {
...
void CameraView::Update(const PlayerArray &players) {
    ...
    if (cameraScaleX > cameraScaleY && cameraScaleX > 1)
        m_camera.zoom = m_baseZoom / cameraScaleX;
    else if (cameraScaleY > 1)
        m_camera.zoom = m_baseZoom / cameraScaleY;
    else
        m_camera.zoom = m_baseZoom;

    if (m_camera.zoom < m_baseZoom / 1.5)
        m_camera.zoom = m_baseZoom / 1.5;
