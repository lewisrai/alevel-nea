...
class Player : public Entity {
  public:
    // Init
    Player(const SoundArray &sounds, bool player0, int keyUp, int keyLeft, int keyRight,
    ...
    void UpdateReceived(const BlockList &blocks, const MonsterList &monsters, const Wall &wall,
                        float &playersHealth);
    ...
    // Setter - sets position to start of the level and gives player extra
    // immunity
    void Reset();
...
class Tether {
  public:
    // Method - draw to screen
    void Draw() const;
    // Method - check if players are too far from each other + set colour and
    // thickness of line
